...
class RecipeSerializer(serializers.ModelSerializer):
    ...
    tag = TagSerializer(many=True, required=False) # make tags as optional part of recipe.
    # Link ingredients to recipe API.
    ingredient = IngredientSerializer(many=True, required=False) # make ingredient as optional part of recipe.
    class Meta:
        model = Recipe
        fields = ['id','title','time_minutes','price','link','tag','ingredient','image','recipe_procedure']
        read_only_fields = ['id']
...
class RecipeDetailSerializer(RecipeSerializer):
    ...
    def _get_or_create_tags(self, tags, ingredients, recipe):
        ...
        for ingredient in ingredients:
            ingredient_exists = Ingredients.objects.filter(name=ingredient['name'], user = auth_user)
