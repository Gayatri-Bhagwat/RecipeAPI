...
class TestRecipeAdminSection(TestCase):
    ...
    def setUp(self):
        self.user_data = {'email':'recipeTest@gmail.com','password':'RecipeTest123','name':'RecipeTestUser',
                          'created_at':timezone.now(),'updated_at':timezone.now()}
        self.user = create_user(**self.user_data)
        self.user_two_data = {'email':'recipeTest2@gmail.com','password':'RecipeTest2123!','name':'RecipeTestUser2',
                              'created_at':timezone.now(),'updated_at':timezone.now()}
        self.user_two = create_user(**self.user_two_data)
        self.recipe = Recipe.objects.create(user=self.user,title="Recipe1",description="A refreshing drink",
        time_minutes=4,link="Link-1",price=25, created_at=timezone.now(), updated_at=timezone.now())
        self.recipe2 = Recipe.objects.create(user=self.user_two,title="Recipe2",description="A refreshing drink made "
        "with coffee, suger, milk",time_minutes=4,link="Link-2",price=50, created_at=timezone.now(),
    ...
    def test_admin_section_contains_required_number_of_fields_in_list_display(self):
        ...
        expected_fields = ['id','user','title','description','time_minutes','price','link','recipe_procedure']
    ...
    def test_admin_section_contains_required_filters_in_list_display(self):
        ...
        expected_filters = ['user','title','price']
